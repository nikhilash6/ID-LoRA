...
class XFormersAttention(AttentionCallable):
    def __call__(
        ...
        # Cast mask to v.dtype to match q/k/v (xformers requires matching dtypes)
        if mask is not None:
...
class AttentionFunction(Enum):
    ...
    def __call__(
        ...
        if self is AttentionFunction.PYTORCH:
            ...
            # Fall back to PyTorch if mask is provided (xformers has strict alignment requirements)
            if mask is not None:
                return PytorchAttention()(q, k, v, heads, mask)
