...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def forward(  # noqa: PLR0915
        ...
        if run_ax:
            ...
            # Use audio_context_attn_mask if provided (for blocking ref audio from text),
            # otherwise fall back to context_mask (standard text padding mask)
            audio_text_mask = (
                audio.audio_context_attn_mask
                if audio.audio_context_attn_mask is not None
                else audio.context_mask
            )
            ax = ax + self.audio_attn2(rms_norm(ax, eps=self.norm_eps), context=audio.context, mask=audio_text_mask)
            ...
            if run_a2v:
                ...
                vx = vx + (
                    self.audio_to_video_attn(
                        vx_scaled,
                        context=ax_scaled,
                        mask=video.a2v_cross_attention_mask,
            ...
            if run_v2a:
                ...
                ax = ax + (
                    self.video_to_audio_attn(
                        ax_scaled,
                        context=vx_scaled,
                        mask=audio.v2a_cross_attention_mask,
