...
def preprocess_dataset(  # noqa: PLR0913
    ...
    if reference_column:
        ...
        # Process reference video latents (and audio if enabled)
        # Output is named after main_media_column (target) but contains reference video content
        # This creates the correct mapping: target_id.pt -> reference_video_latents
        reference_audio_dir = output_base / "reference_audio_latents" if with_audio else None
        ...
        compute_latents(
            ...
            vae_tiling=vae_tiling,
            with_audio=with_audio,
            audio_output_dir=str(reference_audio_dir) if reference_audio_dir else None,
            skip_existing=skip_existing,
        )
    ...
    if reference_column:
        logger.info(
            "Reference videos processed for IC-LoRA training:\n"
            "  - reference_latents/: Video latents from reference videos, named by target video ID\n"
            "  - reference_audio_latents/: Audio latents from reference videos, named by target video ID"
            if with_audio else
            "  - reference_latents/: Video latents from reference videos, named by target video ID"
        )
    if with_audio:
...
def main(  # noqa: PLR0913
    ...
    skip_existing: bool = typer.Option(
        default=False,
        help="Skip processing if output file already exists (for resuming interrupted preprocessing)",
    ),
) -> None:
    ...
    Examples:
        ...
        python scripts/process_dataset.py dataset.json --resolution-buckets 768x512x97 \\
            --model-path /path/to/ltx2.safetensors --text-encoder-path /path/to/gemma \\
            --with-audio
        # Process dataset for AV-IC-LoRA (audio-video identity transfer) training
        # This creates: latents/, audio_latents/, reference_latents/, reference_audio_latents/
        # Reference latents are named by TARGET video ID but contain REFERENCE video content
        python scripts/process_dataset.py dataset.json --resolution-buckets 512x512x121 \\
            --model-path /path/to/ltx2.safetensors --text-encoder-path /path/to/gemma \\
            --reference-column "reference_path" --with-audio
    """
