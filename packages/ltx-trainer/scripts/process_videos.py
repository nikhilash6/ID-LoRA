...
class MediaDataset(Dataset):
    ...
    def _extract_audio(video_path: Path, target_duration: float) -> dict[str, torch.Tensor | int] | None:
        ...
        try:
            ...
            # Convert mono to stereo by duplicating the channel (Audio VAE expects 2 channels)
            if waveform.shape[0] == 1:
                logger.info(f"Converting mono to stereo for {video_path}")
                waveform = waveform.repeat(2, 1)  # [1, samples] -> [2, samples]
            
            logger.debug(f"Audio waveform shape: {waveform.shape}, sample_rate: {sample_rate}")
...
def _load_metadata(dataset_file: Path) -> list[dict]:
    """Load metadata from a JSON, JSONL, or CSV file."""
...
def _write_temp_metadata(data: list[dict], original_file: Path) -> Path:
    """Write filtered metadata to a temporary file with the same format as the original."""
    import tempfile
    
    suffix = original_file.suffix
    # Create temp file in the same directory as original to preserve relative paths
    temp_dir = original_file.parent
    
    if suffix == ".json":
        temp_path = temp_dir / f".temp_filtered_{original_file.stem}.json"
        with open(temp_path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)
    elif suffix == ".jsonl":
        temp_path = temp_dir / f".temp_filtered_{original_file.stem}.jsonl"
        with open(temp_path, "w", encoding="utf-8") as f:
            for item in data:
                f.write(json.dumps(item) + "\n")
    elif suffix == ".csv":
        temp_path = temp_dir / f".temp_filtered_{original_file.stem}.csv"
...
def filter_unprocessed_items(
    ...
    """
    Filter dataset to only include items that need processing.
    
    Returns:
        tuple of (filtered_dataset_path, original_count, filtered_count)
    """
    dataset_file = Path(dataset_file)
    output_path = Path(output_dir)
    audio_output_path = Path(audio_output_dir) if audio_output_dir else None
    
    # Load all metadata
    data = _load_metadata(dataset_file)
    original_count = len(data)
    
    # Filter to only unprocessed items
    unprocessed = []
    for item in data:
        media_path = Path(item[main_media_column])
        output_file = output_path / media_path.with_suffix(".pt")
        
        video_exists = output_file.exists()
        audio_exists = True  # Default to true if not using audio
        
        if with_audio and audio_output_path:
            audio_file = audio_output_path / media_path.with_suffix(".pt")
            audio_exists = audio_file.exists()
        
        # Only include if either video or audio (when enabled) doesn't exist
    ...
    if filtered_count == 0:
        ...
        return dataset_file, original_count, 0
    
    # Write filtered data to temp file
    filtered_file = _write_temp_metadata(unprocessed, dataset_file)
    logger.info(f"Pre-filtered dataset: {original_count} -> {filtered_count} items ({original_count - filtered_count} skipped)")
    
    return filtered_file, original_count, filtered_count
...
def compute_latents(  # noqa: PLR0913, PLR0915
    ...
    # Pre-filter dataset if skip_existing is enabled (more efficient than post-filtering)
    if skip_existing:
        effective_main_media_column = main_media_column or video_column
        filtered_file, original_count, filtered_count = filter_unprocessed_items(
            dataset_file=dataset_file,
            main_media_column=effective_main_media_column,
        ...
        if filtered_count == 0:
            logger.info("All items already processed, exiting early")
            return
        
        # Use filtered file for dataset
        if filtered_file != Path(dataset_file):
            temp_dataset_file = filtered_file
            dataset_file = filtered_file
    ...
    with Progress(
        ...
        for batch in dataloader:
            ...
            for i in range(len(batch["relative_path"])):
                ...
                # Safety net: double-check if we should skip (in case of race conditions or edge cases)
                # Note: The main skip filtering happens before dataset creation for efficiency
                video_exists = output_file.exists()
                audio_exists = True  # default to true if not using audio
                if with_audio:
                    audio_output_file = audio_output_path / output_rel_path
                    audio_exists = audio_output_file.exists()

                if skip_existing and video_exists and audio_exists:
                    continue
    ...
    if temp_dataset_file is not None and temp_dataset_file.exists():
        ...
        logger.debug(f"Cleaned up temp file: {temp_dataset_file}")
...
def encode_audio(
    ...
    logger.debug(f"encode_audio received waveform shape: {waveform.shape}")
    ...
    if waveform.shape[1] == 1:
        logger.info(f"Converting mono to stereo in encode_audio")
        waveform = waveform.repeat(1, 2, 1)  # [B, 1, S] -> [B, 2, S]
    
    logger.debug(f"encode_audio waveform after processing: {waveform.shape}")
