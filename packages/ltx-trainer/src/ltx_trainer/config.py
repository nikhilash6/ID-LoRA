...
from ltx_trainer.training_strategies.audio_ic import AudioICConfig
from ltx_trainer.training_strategies.audio_ic_masked import AudioICMaskedConfig
from ltx_trainer.training_strategies.audio_ref_only_ic import AudioRefOnlyICConfig
from ltx_trainer.training_strategies.audio_video_ic import AudioVideoICConfig
from ltx_trainer.training_strategies.first_frame_av_ic import FirstFrameAVICConfig
from ltx_trainer.training_strategies.negative_pos_av_ic import NegativePosAVICConfig
from ltx_trainer.training_strategies.negative_pos_full_ref_av_ic import NegativePosFullRefAVICConfig
from ltx_trainer.training_strategies.speaker_identity_av_ic import SpeakerIdentityAVICConfig
...
TrainingStrategyConfig = Annotated[
    ...
    | Annotated[AudioVideoICConfig, Tag("audio_video_ic")]
    | Annotated[AudioICConfig, Tag("audio_ic")]
    | Annotated[AudioICMaskedConfig, Tag("audio_ic_masked")]
    | Annotated[AudioRefOnlyICConfig, Tag("audio_ref_only_ic")]
    | Annotated[FirstFrameAVICConfig, Tag("first_frame_av_ic")]
    | Annotated[NegativePosAVICConfig, Tag("negative_pos_av_ic")]
    | Annotated[NegativePosFullRefAVICConfig, Tag("negative_pos_full_ref_av_ic")]
    | Annotated[SpeakerIdentityAVICConfig, Tag("speaker_identity_av_ic")],
...
class ValidationConfig(ConfigBaseModel):
    ...
    images: list[str | None] | None = Field(
        ...
        "One image path (or null) must be provided for each validation prompt. "
        "Use null for prompts that should not use first-frame image conditioning.",
    )
    ...
    target_videos: list[str] | None = Field(
        default=None,
        description="List of ground truth target video paths for comparison in W&B. "
        "These are uploaded at training start for visual comparison with generated outputs.",
    )
    ...
    compute_metrics: bool = Field(
        default=False,
        description="Compute face and speaker similarity metrics during validation. "
        "Compares generated samples against reference_videos and logs to W&B.",
    )

    speaker_metric: Literal["ecapa_tdnn", "wavlm_ecapa", "none"] | None = Field(
        default="wavlm_ecapa",
        description="Speaker similarity metric: 'wavlm_ecapa' (WavLM+ECAPA-TDNN SOTA 0.431%% EER, recommended), "
        "'ecapa_tdnn' (SpeechBrain ~0.8%% EER), or 'none'/null to disable.",
    )

    face_metric: Literal["arcface", "none"] | None = Field(
        default="arcface",
        description="Face similarity metric: 'arcface' (InsightFace ArcFace) or 'none'/null to disable.",
    )

    clap_metric: Literal["clap", "none"] | None = Field(
        default=None,
        description="CLAP caption-to-audio similarity metric: 'clap' (LAION CLAP) or 'none'/null to disable. "
        "Extracts [ENVIRONMENT_SOUNDS] and [SPEAKING_STYLE] from prompts/captions and compares to generated audio.",
    )

    captions: list[str] | None = Field(
        default=None,
        description="List of captions for CLAP metrics. Each caption should contain [ENVIRONMENT_SOUNDS] and/or "
        "[SPEAKING_STYLE] sections. If not provided, uses prompts for CLAP. One caption per validation prompt.",
    )

    mask_ref_audio_to_text: bool | None = Field(
        default=None,
        description="If set, overrides training strategy for validation only: block reference audio from attending to text. None = use strategy value.",
    )

    mask_cross_attention_to_reference: bool | None = Field(
        default=None,
        description="If set, overrides training strategy for validation only: block video from attending to reference audio. None = use strategy value.",
    )

    @field_validator("images")
    @classmethod
    def validate_images(cls, v: list[str | None] | None, info: ValidationInfo) -> list[str | None] | None:
        """Validate that number of images (if provided) matches number of prompts.
        
        Supports null entries in the list for prompts that don't use image conditioning.
        """
        if v is None:
            return None
    ...
    @classmethod
    def validate_captions(cls, v: list[str] | None, info: ValidationInfo) -> list[str] | None:
        """Validate that number of captions (if provided) matches number of prompts."""
        if v is None:
            return None

        num_prompts = len(info.data.get("prompts", []))
        if v is not None and len(v) != num_prompts:
            raise ValueError(f"Number of captions ({len(v)}) must match number of prompts ({num_prompts})")
...
class LtxTrainerConfig(ConfigBaseModel):
    ...
    def validate_strategy_compatibility(self) -> "LtxTrainerConfig":
        ...
        # IC strategies that require reference videos for validation
        ic_strategies = ("video_to_video", "audio_video_ic", "audio_ic", "audio_ic_masked", "first_frame_av_ic", "negative_pos_av_ic", "negative_pos_full_ref_av_ic")

        # Check that reference videos are provided when using IC strategies
        if (
            self.training_strategy.name in ic_strategies
            and self.validation.interval
            and not self.validation.reference_videos
        ):
            raise ValueError(
                f"reference_videos must be provided in validation config when using {self.training_strategy.name} strategy"
            )
        ...
        # Check that LoRA mode is used for IC strategies
        if self.training_strategy.name in ic_strategies and self.model.training_mode != "lora":
            raise ValueError(f"Training mode must be 'lora' when using {self.training_strategy.name} strategy")
