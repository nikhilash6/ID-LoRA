...
def load_model(
    ...
    Args:
        ...
        with_audio_vae_encoder: Whether to load the audio VAE encoder (for reference audio conditioning)
    ...
    # Load audio VAE encoder (for reference audio conditioning in IC-LoRA)
    audio_vae_encoder = None
    if with_audio_vae_encoder:
        logger.debug("Loading audio VAE encoder...")
        # Audio VAE encoder uses float32 for better quality
        audio_vae_encoder = load_audio_vae_encoder(checkpoint_path, torch_device, torch.float32)
