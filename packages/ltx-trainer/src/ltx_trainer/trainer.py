...
from ltx_trainer.datasets import PrecomputedDataset, pad_audio_latents_collate_fn
...
from ltx_trainer.training_strategies import LossResult, get_training_strategy
...
class LtxvTrainer:
    def __init__(self, trainer_config: LtxTrainerConfig) -> None:
        ...
        self._move_speaker_adapter_to_device()
        self._dataset = None
        self._global_step = -1
        self._checkpoint_paths = []
        self._speaker_metric = None  # Lazy loaded when compute_metrics is enabled
        self._face_metric = None  # Lazy loaded when compute_metrics is enabled
        self._clap_metric = None  # Lazy loaded when compute_metrics is enabled
        self._init_wandb()
        self._upload_validation_videos_to_wandb()
    ...
    def train(  # noqa: PLR0912, PLR0915
        ...
        with progress:
            ...
            if cfg.validation.interval and not cfg.validation.skip_initial_validation:
                ...
                if IS_MAIN_PROCESS and sampled_videos_paths:
                    if self._config.wandb.log_validation_videos:
                        self._log_validation_samples(sampled_videos_paths, cfg.validation.prompts)
                    # Compute and log validation metrics
                    if self._config.validation.compute_metrics:
                        metrics = self._compute_validation_metrics(sampled_videos_paths)
                        if metrics and self._wandb_run is not None:
                            self._wandb_run.log(metrics, step=0)
            ...
            for step in range(cfg.optimization.steps * cfg.optimization.gradient_accumulation_steps):
                ...
                with self._accelerator.accumulate(self._transformer):
                    ...
                    loss_result = self._training_step(batch)
                    self._accelerator.backward(loss_result.total)

                    # Debug: Log speaker adapter gradients
                    if self._speaker_adapter is not None and self._global_step % 10 == 0:
                        adapter_grads = []
                        for name, param in self._speaker_adapter.named_parameters():
                            if param.grad is not None:
                                adapter_grads.append((name, param.grad.norm().item()))
                        if adapter_grads:
                            grad_info = ", ".join([f"{n}: {g:.6f}" for n, g in adapter_grads[:3]])
                            logger.debug(f"Speaker adapter gradients (step {self._global_step}): {grad_info}")
                        else:
                            logger.warning(f"Speaker adapter has NO gradients at step {self._global_step}!")
                    ...
                    if IS_MAIN_PROCESS and is_optimization_step:
                        metrics = {
                            "train/loss": loss_result.total.item(),
                            "train/video_loss": loss_result.video.item(),
                            "train/learning_rate": current_lr,
                            "train/step_time": step_time,
                            "train/global_step": self._global_step,
                        }
                        if loss_result.audio is not None:
                            metrics["train/audio_loss"] = loss_result.audio.item()
                    ...
                    if disable_progress_bars and IS_MAIN_PROCESS and self._global_step % 20 == 0:
                        ...
                        logger.info(
                            ...
                            f"Loss: {loss_result.total.item():.4f}, LR: {current_lr:.2e}, "
    ...
    def _training_step(self, batch: dict[str, dict[str, Tensor]]) -> LossResult:
        ...
        # Use strategy to compute loss (returns LossResult with individual components)
        loss_result = self._training_strategy.compute_loss(video_pred, audio_pred, model_inputs)
    ...
    def _load_models(self) -> None:
        ...
        # Check if we need audio VAE encoder (for reference audio conditioning in IC-LoRA)
        # This is needed when we have reference videos and audio generation enabled
        need_audio_encoder = (
            self._config.validation.reference_videos is not None and load_audio
        )
        ...
        components = load_ltx_model(
            ...
            with_audio_vae_encoder=need_audio_encoder,  # Needed for reference audio in IC-LoRA
        ...
        # Create AudioProcessor for waveform-to-spectrogram conversion if audio encoder is loaded
    ...
    def _collect_trainable_params(self) -> None:
        ...
        # Add speaker identity adapter parameters if present
        # Note: The adapter will be moved to GPU later in _move_speaker_adapter_to_device()
        # which is called after _prepare_models_for_training() when the transformer is on GPU
        self._speaker_adapter = None
        if hasattr(self._training_strategy, "speaker_adapter") and self._training_strategy.speaker_adapter is not None:
            self._speaker_adapter = self._training_strategy.speaker_adapter
            adapter_params = self._speaker_adapter.get_trainable_parameters()
            self._trainable_params.extend(adapter_params)
            adapter_param_count = sum(p.numel() for p in adapter_params)
            logger.info(f"Added speaker identity adapter: {adapter_param_count:,} trainable parameters")
    ...
    def _load_full_checkpoint(self, checkpoint_path: Path) -> None:
        ...
        # Try to load speaker adapter checkpoint if it exists
        self._load_speaker_adapter_checkpoint(checkpoint_path)
    ...
    def _load_speaker_adapter_checkpoint(self, lora_checkpoint_path: Path) -> None:
        """Load speaker identity adapter checkpoint if it exists alongside LoRA checkpoint."""
        if self._speaker_adapter is None:
            return

        # Derive adapter checkpoint path from LoRA checkpoint path
        # e.g., lora_weights_step_01000.safetensors -> speaker_adapter_step_01000.safetensors
        checkpoint_dir = lora_checkpoint_path.parent
        step_str = lora_checkpoint_path.stem.split("_step_")[-1]  # e.g., "01000"
        adapter_filename = f"speaker_adapter_step_{step_str}.safetensors"
        adapter_path = checkpoint_dir / adapter_filename

        if not adapter_path.exists():
            logger.debug(f"No speaker adapter checkpoint found at {adapter_path}")
            return

        logger.info(f"📥 Loading speaker adapter checkpoint from {adapter_path}")
        adapter_state = load_file(adapter_path)
        # Use strict=False since we only save trainable params (not frozen encoder)
        self._speaker_adapter.load_state_dict(adapter_state, strict=False)
        logger.info("✅ Speaker adapter checkpoint loaded successfully")
    ...
    def _move_speaker_adapter_to_device(self) -> None:
        """Move speaker identity adapter to the same device as the transformer.
        
        This must be called after _prepare_models_for_training() when the transformer
        is on GPU.
        """
        if self._speaker_adapter is None:
            return
            
        # Get device from transformer (now on GPU after accelerator.prepare)
        device = next(self._transformer.parameters()).device
        dtype = torch.bfloat16  # Use same dtype as training
        
        self._speaker_adapter = self._speaker_adapter.to(device=device, dtype=dtype)
        # Update the strategy's reference to point to the moved adapter
        self._training_strategy._speaker_adapter = self._speaker_adapter
        
        # Log initial weight statistics for debugging
        weight_norms = []
        for name, param in self._speaker_adapter.named_parameters():
            weight_norms.append(f"{name}: {param.norm().item():.4f}")
        logger.info(f"Moved speaker identity adapter to {device}")
        logger.info(f"Initial adapter weight norms: {', '.join(weight_norms)}")
    ...
    def _init_dataloader(self) -> None:
        ...
        # Use custom collate function to handle variable-length audio latents
        # This is necessary because audio latents from different sources (e.g., audio_latents_clean)
        # may have slightly different sequence lengths (e.g., 120, 121, 122 time steps)
        collate_fn = pad_audio_latents_collate_fn if self._training_strategy.requires_audio else None
    ...
    def _sample_videos(self, progress: TrainingProgress) -> list[Path] | None:
        ...
        # Get speaker identity adapter if available (from speaker_identity_av_ic strategy)
        speaker_adapter = getattr(self._training_strategy, "_speaker_adapter", None)
        ...
        sampler = ValidationSampler(
            ...
            audio_encoder=self._audio_encoder if (generate_audio and use_reference_videos) else None,
            ...
            speaker_identity_adapter=speaker_adapter,
        )
        ...
        for prompt_idx, prompt in enumerate(self._config.validation.prompts):
            ...
            # Load conditioning image if provided (supports per-prompt null values)
            ...
            # Load reference video and audio if provided (for IC-LoRA)
            reference_video = None
            reference_audio = None
            reference_audio_sample_rate = 16000  # Default sample rate
            if use_reference_videos:
                ...
                # Extract audio from reference video if audio encoder is available
                if generate_audio and self._audio_encoder is not None:
                    import torchaudio
                    try:
                        waveform, sample_rate = torchaudio.load(str(ref_video_path))
                        reference_audio = waveform
                        reference_audio_sample_rate = sample_rate
                    except Exception as e:
                        logger.warning(f"Could not extract audio from reference video {ref_video_path}: {e}")
            ...
            # Check if this is audio-only IC strategy (no video IC, only audio IC)
            audio_only_ic = self._config.training_strategy.name in ("audio_ic", "audio_ic_masked", "audio_ref_only_ic")
            # Check if this is first-frame-only IC strategy (only first frame of reference video for IC, full audio)
            # Both first_frame_av_ic and negative_pos_av_ic use first-frame reference video
            reference_first_frame_only = self._config.training_strategy.name in ("first_frame_av_ic", "negative_pos_av_ic")
            # Check if negative reference positions should be used (for clear ref/target separation)
            # Can be strategy-wide (negative_pos_*) or per-config (audio_ic_masked)
            use_negative_ref_positions = (
                self._config.training_strategy.name in ("negative_pos_av_ic", "negative_pos_full_ref_av_ic")
                or getattr(self._config.training_strategy, "use_negative_ref_positions", False)
            )
            # Check if ref audio should be masked from attending to text
            # audio_ic_masked uses "mask_ref_audio_to_text", audio_ref_only_ic uses "mask_reference_from_text_attention"
            _mask_ref_audio_to_text = (
                getattr(self._config.training_strategy, "mask_ref_audio_to_text", False)
                or getattr(self._config.training_strategy, "mask_reference_from_text_attention", False)
            )
            # Check if cross-attention to reference should be masked (blocks video from attending to ref audio)
            _mask_cross_attention_to_reference = getattr(self._config.training_strategy, "mask_cross_attention_to_reference", False)
            # Validation overrides: if set in validation config, use those for inference only
            mask_ref_audio_to_text = (
                self._config.validation.mask_ref_audio_to_text
                if self._config.validation.mask_ref_audio_to_text is not None
                else _mask_ref_audio_to_text
            )
            mask_cross_attention_to_reference = (
                self._config.validation.mask_cross_attention_to_reference
                if self._config.validation.mask_cross_attention_to_reference is not None
                else _mask_cross_attention_to_reference
            )
            # Check if audio concatenation should be used (speaker_identity_av_ic can disable it)
            use_audio_concat = getattr(self._config.training_strategy, "use_audio_concat", True)
            # Check if speaker identity injection is enabled
            use_speaker_identity = getattr(self._config.training_strategy, "speaker_identity_enabled", False)

            # Get zero position flags from training strategy config
            zero_ref_positions = getattr(self._config.training_strategy, "zero_ref_positions", False)
            zero_video_ref_positions = getattr(self._config.training_strategy, "zero_video_ref_positions", False)
            zero_audio_ref_positions = getattr(self._config.training_strategy, "zero_audio_ref_positions", False)
            zero_all_ref_positions = getattr(self._config.training_strategy, "zero_all_ref_positions", False)
            ...
            gen_config = GenerationConfig(
                ...
                audio_only_ic=audio_only_ic,
                reference_first_frame_only=reference_first_frame_only,
                use_audio_concat=use_audio_concat,
                use_speaker_identity=use_speaker_identity,
                ...
                zero_ref_positions=zero_ref_positions,
                zero_video_ref_positions=zero_video_ref_positions,
                zero_audio_ref_positions=zero_audio_ref_positions,
                zero_all_ref_positions=zero_all_ref_positions,
                use_negative_ref_positions=use_negative_ref_positions,
                mask_ref_audio_to_text=mask_ref_audio_to_text,
                mask_cross_attention_to_reference=mask_cross_attention_to_reference,
            )
    ...
    def _compute_face_similarity_with_crop(
        self,
        reference_video: Path,
        generated_video: Path,
        crop_right_half: bool = False,
    ) -> float | None:
        """Compute face similarity, optionally cropping the right half of generated video.

        When include_reference_in_output is True, generated videos are side-by-side
        (reference on left, generated on right). We need to crop to the right half
        to avoid comparing reference face to reference face.

        Args:
            reference_video: Path to reference video (single video, not side-by-side)
            generated_video: Path to generated video (may be side-by-side)
            crop_right_half: If True, crop to right half before extracting face

        Returns:
            Face similarity score or None if face detection failed
        """
        if self._face_metric is None:
            return None

        try:
            import cv2
        except ImportError:
            logger.warning("OpenCV not available for face cropping")
            return self._face_metric.compute_similarity(reference_video, generated_video)

        if not crop_right_half:
            return self._face_metric.compute_similarity(reference_video, generated_video)

        # Extract face embedding from reference video (no crop needed)
        ref_embedding = self._face_metric.extract_embedding_multi_frame(reference_video)
        if ref_embedding is None:
            return None

        # Extract face embedding from right half of generated video
        import numpy as np
        ...
        # Sample frames evenly (same as extract_embedding_multi_frame)
        num_frames = 5
        ...
        for idx in frame_indices:
            ...
            # Crop to right half (generated portion)
            right_half = frame[:, width // 2:, :]

            # Detect face in cropped frame
            faces = self._face_metric._model.get(right_half)
            if faces:
        ...
        gen_embedding = np.mean(embeddings, axis=0)
    ...
    def _compute_validation_metrics(self, video_paths: list[Path]) -> dict[str, float]:
        """Compute face and speaker similarity metrics for validation samples.

        Compares each generated video against both reference videos (conditioning source)
        and target videos (ground truth) using ArcFace (face) and ECAPA-TDNN (speaker).

        Note: If include_reference_in_output is True, generated videos are side-by-side.
        For face metrics, we crop to the right half to avoid comparing reference to reference.

        Args:
            video_paths: List of paths to generated validation videos

        Returns:
            Dictionary with metric names and values for W&B logging
        """
        import numpy as np

        if not self._config.validation.compute_metrics:
            return {}

        reference_videos = self._config.validation.reference_videos
        target_videos = self._config.validation.target_videos

        if not reference_videos:
            logger.warning("compute_metrics enabled but no reference_videos configured")
            return {}

        if not video_paths:
            return {}

        # Check if videos are side-by-side (need to crop for face metrics)
        is_side_by_side = self._config.validation.include_reference_in_output

        # Lazy load metric models based on config
        speaker_metric_type = self._config.validation.speaker_metric
        if self._speaker_metric is None and speaker_metric_type and speaker_metric_type != "none":
            try:
                if speaker_metric_type == "wavlm_ecapa":
                    from ltx_trainer.metrics import WavLMSpeakerSimilarityMetric
                    logger.info("Loading WavLM+ECAPA-TDNN model for speaker similarity (SOTA)...")
                    self._speaker_metric = WavLMSpeakerSimilarityMetric(device=str(self._accelerator.device))
                else:  # ecapa_tdnn
                    from ltx_trainer.metrics import SpeakerSimilarityMetric
                    logger.info("Loading ECAPA-TDNN model for speaker similarity...")
                    self._speaker_metric = SpeakerSimilarityMetric(device=str(self._accelerator.device))
            except ImportError as e:
                logger.warning(f"Could not load speaker metric ({speaker_metric_type}): {e}")

        face_metric_type = self._config.validation.face_metric
        if self._face_metric is None and face_metric_type and face_metric_type != "none":
            try:
                from ltx_trainer.metrics import FaceSimilarityMetric
                logger.info("Loading ArcFace model for face similarity...")
                self._face_metric = FaceSimilarityMetric(device=str(self._accelerator.device))
            except ImportError as e:
                logger.warning(f"Could not load face metric: {e}. Install insightface.")

        clap_metric_type = self._config.validation.clap_metric
        if self._clap_metric is None and clap_metric_type and clap_metric_type != "none":
            try:
                from ltx_trainer.metrics import CLAPSimilarityMetric
                logger.info("Loading CLAP model for caption-audio similarity...")
                self._clap_metric = CLAPSimilarityMetric(device=str(self._accelerator.device))
            except ImportError as e:
                logger.warning(f"Could not load CLAP metric: {e}. Install transformers.")

        # Compute metrics for each generated video
        # Against reference videos (conditioning source)
        speaker_similarities_ref = []
        face_similarities_ref = []
        # Against target videos (ground truth)
        speaker_similarities_target = []
        face_similarities_target = []
        # CLAP similarities (caption-to-audio)
        clap_env_similarities = []
        clap_speaking_similarities = []

        # Get prompts/captions for CLAP computation
        # Use captions if provided, otherwise fall back to prompts
        prompts = self._config.validation.prompts
        captions = self._config.validation.captions or prompts

        for i, gen_path in enumerate(video_paths):
            # === Compare against REFERENCE video ===
            ref_idx = i if i < len(reference_videos) else 0
            ref_video = Path(reference_videos[ref_idx])

            if ref_video.exists():
                # Speaker similarity (audio) vs reference
                # Audio is NOT affected by side-by-side (only generated audio is saved)
                if self._speaker_metric is not None:
                    try:
                        speaker_sim = self._speaker_metric.compute_similarity(ref_video, gen_path)
                        speaker_similarities_ref.append(speaker_sim)
                    except Exception as e:
                        logger.warning(f"Speaker similarity (ref) failed for {gen_path.name}: {e}")

                # Face similarity (video frames) vs reference
                # If side-by-side, crop to right half (generated portion)
                if self._face_metric is not None:
                    try:
                        face_sim = self._compute_face_similarity_with_crop(
                            ref_video, gen_path, crop_right_half=is_side_by_side
                        )
                        if face_sim is not None:
                            face_similarities_ref.append(face_sim)
                    except Exception as e:
                        logger.warning(f"Face similarity (ref) failed for {gen_path.name}: {e}")
            else:
                logger.warning(f"Reference video not found: {ref_video}")

            # === Compare against TARGET video (ground truth) ===
            if target_videos:
                target_idx = i if i < len(target_videos) else 0
                target_video = Path(target_videos[target_idx])

                if target_video.exists():
                    # Speaker similarity (audio) vs target
                    if self._speaker_metric is not None:
                        try:
                            speaker_sim = self._speaker_metric.compute_similarity(target_video, gen_path)
                            speaker_similarities_target.append(speaker_sim)
                        except Exception as e:
                            logger.warning(f"Speaker similarity (target) failed for {gen_path.name}: {e}")

                    # Face similarity (video frames) vs target
                    # If side-by-side, crop to right half (generated portion)
                    if self._face_metric is not None:
                        try:
                            face_sim = self._compute_face_similarity_with_crop(
                                target_video, gen_path, crop_right_half=is_side_by_side
                            )
                            if face_sim is not None:
                                face_similarities_target.append(face_sim)
                        except Exception as e:
                            logger.warning(f"Face similarity (target) failed for {gen_path.name}: {e}")
                else:
                    logger.warning(f"Target video not found: {target_video}")

            # === CLAP similarity (caption-to-audio) ===
            if self._clap_metric is not None:
                caption_idx = i if i < len(captions) else 0
                caption = captions[caption_idx] if captions else ""

                if caption:
                    # CLAP similarity for generated video
                    try:
                        env_sim = self._clap_metric.compute_similarity_from_video(
                            gen_path, caption, caption_type="env", use_cosine=True
                        )
                        if env_sim is not None:
                            clap_env_similarities.append(env_sim)
                    except Exception as e:
                        logger.warning(f"CLAP env similarity (gen) failed for {gen_path.name}: {e}")

                    try:
                        speaking_sim = self._clap_metric.compute_similarity_from_video(
                            gen_path, caption, caption_type="speaking", use_cosine=True
                        )
                        if speaking_sim is not None:
                            clap_speaking_similarities.append(speaking_sim)
                    except Exception as e:
                        logger.warning(f"CLAP speaking similarity (gen) failed for {gen_path.name}: {e}")

                    # Note: Target CLAP similarity not computed during training validation
                    # since it's constant (same target videos and captions every time).
                    # Use compute_all_metrics.py for offline evaluation with target baselines.

        # Aggregate metrics
        metrics = {}

        # Reference-based metrics (include metric type in name for clarity)
        if speaker_similarities_ref:
            key = f"val/speaker_similarity_ref_{speaker_metric_type}"
            metrics[key] = float(np.mean(speaker_similarities_ref))
            if len(speaker_similarities_ref) > 1:
                metrics[f"{key}_std"] = float(np.std(speaker_similarities_ref))
            logger.info(f"Speaker similarity (ref, {speaker_metric_type}): {metrics[key]:.4f}")

        if face_similarities_ref:
            key = f"val/face_similarity_ref_{face_metric_type}"
            metrics[key] = float(np.mean(face_similarities_ref))
            if len(face_similarities_ref) > 1:
                metrics[f"{key}_std"] = float(np.std(face_similarities_ref))
            logger.info(f"Face similarity (ref, {face_metric_type}): {metrics[key]:.4f}")

        # Target-based metrics (include metric type in name for clarity)
        if speaker_similarities_target:
            key = f"val/speaker_similarity_target_{speaker_metric_type}"
            metrics[key] = float(np.mean(speaker_similarities_target))
            if len(speaker_similarities_target) > 1:
                metrics[f"{key}_std"] = float(np.std(speaker_similarities_target))
            logger.info(f"Speaker similarity (target, {speaker_metric_type}): {metrics[key]:.4f}")

        if face_similarities_target:
            key = f"val/face_similarity_target_{face_metric_type}"
            metrics[key] = float(np.mean(face_similarities_target))
            if len(face_similarities_target) > 1:
                metrics[f"{key}_std"] = float(np.std(face_similarities_target))
            logger.info(f"Face similarity (target, {face_metric_type}): {metrics[key]:.4f}")

        # CLAP metrics (caption-to-audio)
        if clap_env_similarities:
            key = "val/clap_env_similarity"
            metrics[key] = float(np.mean(clap_env_similarities))
            if len(clap_env_similarities) > 1:
                metrics[f"{key}_std"] = float(np.std(clap_env_similarities))
            logger.info(f"CLAP env similarity (generated): {metrics[key]:.4f}")

        if clap_speaking_similarities:
            key = "val/clap_speaking_similarity"
            metrics[key] = float(np.mean(clap_speaking_similarities))
            if len(clap_speaking_similarities) > 1:
                metrics[f"{key}_std"] = float(np.std(clap_speaking_similarities))
            logger.info(f"CLAP speaking similarity (generated): {metrics[key]:.4f}")

        # Offload metric models to CPU to free GPU memory for training
        # This is critical to prevent OOM when validation runs during training
        self._offload_metrics_to_cpu()

        return metrics

    def _offload_metrics_to_cpu(self):
        """Offload metric models to CPU to free GPU memory.

        This is called after computing validation metrics to ensure the GPU memory
        is available for training. Without this, the metric models (WavLM ~1.2GB,
        ArcFace ~1GB, CLAP ~500MB) can cause OOM when combined with training memory usage.
        """
        if self._speaker_metric is not None:
            self._speaker_metric.offload_to_cpu()
        if self._face_metric is not None:
            self._face_metric.offload_to_cpu()
        if self._clap_metric is not None:
            self._clap_metric.offload_to_cpu()
    ...
    def _save_checkpoint(self) -> Path | None:
        ...
        # Save speaker identity adapter if present (only trainable params, not frozen encoder)
        if self._speaker_adapter is not None:
            adapter_filename = f"speaker_adapter_step_{self._global_step:05d}.safetensors"
            adapter_path = save_dir / adapter_filename
            # Only save trainable parameters (speaker_proj + output_norm), not the frozen WavLM encoder
            adapter_state = {k: v.to(save_dtype) for k, v in self._speaker_adapter.get_trainable_state_dict().items()}
            save_file(adapter_state, adapter_path)
            logger.info(f"💾 Speaker adapter weights for step {self._global_step} saved in {adapter_path.relative_to(self._config.output_dir)}")
    ...
    def _upload_validation_videos_to_wandb(self) -> None:
        """Upload reference and target videos to W&B at training start for comparison."""
        ...
        if validation_config.reference_videos:
            ref_videos = []
            for i, ref_path in enumerate(validation_config.reference_videos):
                path = Path(ref_path)
                if path.exists():
                    caption = f"Reference {i+1}: {path.name}"
                    ref_videos.append(wandb.Video(str(path), caption=caption, fps=25))
            if ref_videos:
                self._wandb_run.log({"reference_videos": ref_videos}, step=0)
                logger.info(f"Uploaded {len(ref_videos)} reference videos to W&B")

        # Upload target videos (ground truth for comparison)
        if validation_config.target_videos:
            target_videos = []
            for i, target_path in enumerate(validation_config.target_videos):
                path = Path(target_path)
                if path.exists():
                    caption = f"Target {i+1}: {path.name}"
                    target_videos.append(wandb.Video(str(path), caption=caption, fps=25))
            if target_videos:
                self._wandb_run.log({"target_videos": target_videos}, step=0)
                logger.info(f"Uploaded {len(target_videos)} target videos to W&B")
