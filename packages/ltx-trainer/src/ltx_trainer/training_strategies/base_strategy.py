...
class ModelInputs:
    ...
    # Speaker identity conditioning (for SpeakerIdentityAVICStrategy)
    speaker_identity_tokens: Tensor | None = None  # [B, num_tokens, hidden_dim]
    ref_audio_seq_len: int | None = None  # For speaker identity: length of reference audio


@dataclass
class LossResult:
    """Container for loss components, allowing individual logging of video and audio losses."""

    total: Tensor  # Combined loss for backward()
    video: Tensor  # Video-only loss
    audio: Tensor | None  # Audio loss (None if audio disabled)
...
class TrainingStrategy(ABC):
    ...
    def compute_loss(
        ...
        Returns:
            LossResult containing total, video, and audio losses
        """
